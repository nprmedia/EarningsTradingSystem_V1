...
def _valid_bar(q: dict) -> bool:
    try:
        o = float(q.get("open", 0))
        h = float(q.get("high", 0))
        low = float(q.get("low", 0))
        c = float(q.get("last", 0))
        if o <= 0 or h <= 0 or low <= 0 or c <= 0:
            return False
        if low > h:
            return False
        span = (h - low) / o if o else 0
        return span < 0.2
    except Exception:
        return False
...
def _from_finnhub(sym: str) -> Optional[dict]:
    ...
    if not q:
        _log(sym, "finnhub", False, (time.time() - t0) * 1000)
        return None
    out = {
...
def fetch_quote_basic(symbol: str) -> Optional[dict]:
    ...
    t0 = time.time()
    q = _yd(s)
    ok = bool(q and _valid_bar(q))
    _log(s, "yahoo_direct", ok, (time.time() - t0) * 1000)
    if ok:
        _MEMO[s] = q
        return q
    t0 = time.time()
    q = _yf(s)
    ok = bool(q and _valid_bar(q))
    _log(s, "yfinance", ok, (time.time() - t0) * 1000)
    if ok:
        _MEMO[s] = q
        return q
    t0 = time.time()
    q = _stq(s)
    ok = bool(q and _valid_bar(q))
    _log(s, "stooq", ok, (time.time() - t0) * 1000)
    if ok:
        _MEMO[s] = q
        return q
    _MEMO[s] = None
