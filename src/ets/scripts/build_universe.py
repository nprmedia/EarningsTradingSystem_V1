    ...
    if os.path.exists(path):
        ...
        with open(path, "r", encoding="utf-8") as f:
            for r in _csv.DictReader(f):
                out.append({"date": r.get("date",""), "symbol": r.get("symbol","").upper(), "session": r.get("session","").lower()})
    return out
...
def build_universe():
    ...
    cal_path = os.path.join(cache_dir, "calendar.csv"); sec_path = os.path.join(cache_dir, "sectors.csv")
    ...
    symbols = {r["symbol"] for r in cal if r["date"] in window and (session=="" or r["session"]==session)}
    ...
    with open("tickers.csv","w",newline="",encoding="utf-8") as f:
        w=csv.writer(f); w.writerow(["symbol"]); w.writerows([[s] for s in keep])
