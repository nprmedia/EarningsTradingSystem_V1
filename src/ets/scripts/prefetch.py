...
def main():
    ...
    with open(os.path.join(cache_dir, "calendar.csv"), "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f); w.writerow(["date","symbol","session"]); w.writerows(cal_rows)
    ...
    print(f"[OK] Prefetched: calendar={len(cal_rows)} sectors_added={len(new_secs)} etf_warmup={len(cfg.get('features', {}).get('sector_etfs', []))}")
